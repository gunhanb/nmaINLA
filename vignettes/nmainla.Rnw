...
Here we use the Atrial dataset which is originally analyzed in \cite{batson2016exploratory} (see \texttt{?Atrialdat}). There are four different covariates available. We only use \texttt{age} covariate to fit a NMA-regression model. Firstly, we delete the study in which \texttt{age} covariate information is not available. Then, centered covariate information can be given by \texttt{covariate = 'age'} as follows:
